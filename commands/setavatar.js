module.exports = {
	...
	exec: (call) => {
		...
		if (!newAvatar)
			return call.message.channel.send('Please rerun the command with a link to the new avatar or attach it to the command message.');

		call.client.user.setAvatar(newAvatar)
			.then(() => call.message.channel.send('Changed the avatar.'),
				() => call.message.channel.send('Failed to change the avatar. Make sure the URL is valid.'));
	}
};