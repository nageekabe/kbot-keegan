module.exports = {
	...
	exec: (call) => {
		...
		let newUsername = call.cut;

		if (!newUsername || newUsername.length < 2 || newUsername.length > 32)
			return call.message.channel.send('Please rerun the command with a valid username between 2 and 32 characters.');

		call.client.user.setUsername(newUsername)
			.then(() => call.message.channel.send('Changed the username.'),
				() => call.message.channel.send('Failed to change the username.'));
	}
};